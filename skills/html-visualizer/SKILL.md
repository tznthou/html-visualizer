...
13. ⭐⭐ **跑自檢指令**（取代過去十來條手打 grep）：
    ...
    一次跑完並依產出類型自動分流。每一項都對應一次真實事故，來龍去脈全文在 `references/incidents.md`；這裡只列它抓什麼、為什麼靜態看不到：

    | 檢查 | 抓什麼 | 為什麼非它不可（日期對應 incidents.md 各段） |
    |---|---|---|
    | 結構完整性 | 標籤平衡、標題有命名 | — |
    | 腳本健檢 | 每個 inline script 跑 `node --check`；JS 抓的元素 id 真的存在 | 08-13：JS 字串混進真換行、整段 SyntaxError，複製鈕按了沒反應。它不在標記裡，在「腳本能不能跑」 |
    | 執行期健檢 | playwright 真的載入頁面抓 pageerror，再按一次複製鈕與預覽鈕看有沒有變 | 09-09：砍掉題目地圖後 `qmap.remove()` 對 null 炸掉，同區塊後面的監聽器全沒掛，語法檢查全綠。靜態檢查永遠有下一種漏法，「載入→有沒有炸→按下去有沒有變」不會漏 |
    | 樣式健檢 | 每個 inline `<style>` 抓多一個 `}`、規則沒收尾、孤兒屬性 | 09-08：用行號切 CSS 切在規則中間，整頁沒樣式；括號左右各 158 剛好平衡。括號平衡量的是數量，合法性要的是結構 |
    ...
    | CJK 斜體 | 與版面健檢同一支腳本：computed `font-style` 非 normal、`font-synthesis` 仍含 `style`、且元素自身的直接文字節點含中文 → `✗` | 09-11：中文沒有 italic 字形，套了只會被幾何傾斜。這條規則本來就確立過、也修乾淨過，交棒到本 skill 時掉了，反而寫進核心風格宣告，實測單頁 17 處。CSS 可能來自任何一層選擇器，靜態 grep 抓不到。09-23 補 `font-synthesis` 判準：設了保險絲的頁面渲染是正體，只讀 `font-style` 會誤報 |
    ...
    - **「未驗證」不等於通過**：找不到 `node`、playwright 或瀏覽器時相關項標 `?`，看到就別當驗過了。pnpm 專案設 `HTML_VISUALIZER_PLAYWRIGHT_ROOT=<repo>/node_modules/.pnpm/playwright@<版本>/node_modules`。趕時間可加 `--no-layout` 跳過真渲染，但那就是沒驗
    - **有 `✗` 就修完再 open**。唯一例外：含 SVG 的頁面，版面健檢對 `<svg>` 報「凸出視窗」時先確認外層是不是 `.figure`（受控橫向捲動），是就屬預期；「文字被切掉」對 SVG 已排除誤報，那條紅字要當真
    - **不要為了讓某項自檢變綠而改東西**：不套已定義的 class 名（套用前先 `rg '\.<名字>\s*\{' <head 檔>` 確認），不對內容欄加 `nowrap`。凡「為了過檢查」而改的，一律親自看渲染結果，不能只信同一支自檢再跑一次通過
    - **版面健檢證不了樣式有生效**：它量座標，樣式全失效時每個元素都還在原位；兩項一起看
    - **不要用行號切 CSS 片段**：複用範本樣式就整段複製到規則邊界，或整份 head 一起帶
    - **表格寫法**：`table-layout: fixed` ＋ `<colgroup>` 明定各欄百分比，只有「A／B」標籤欄可 `nowrap`，手機包 `overflow-x: auto` 整表橫向捲動（範例見 `references/component-library.md` § 方案對照表）
    - **砍範本區塊時對應的 JS 一起砍或加 null 防護**：同一個 script 區塊任何一行炸掉，後面的監聽器全不掛。用腳本產 HTML 要放換行改用 `String.fromCharCode(10)`，不用轉義字元
    - **別再自己現寫 grep**：手打正則出錯會產生假警報
...
| `references/incidents.md` | 想知道某條自檢或規則「為什麼存在」時 — 每項檢查對應的真實事故全文。改規則、加檢查項前先讀；新增檢查項時主檔的表加一列、它加一段 |
